...
{
    public EntityAttachmentRepository(TContext dbContext, IAttachmentService attachmentService, IIdentifierGenerator? identifierGenerator = null)
        : base(dbContext, attachmentService, identifierGenerator)
    {
    }
}
...
public class EntityAttachmentRepository<TContext, TObject, TObjectKey, TEntityAttachment, TEntityAttachmentKey, TSearchObject, TAttachmentKey> : EntityRepository<TContext, TEntityAttachment, TEntityAttachmentKey, TSearchObject>
    where TContext : DbContext
    ...
    where TSearchObject : class, IEntityAttachmentSearchObject<TEntityAttachmentKey, TObjectKey>, new()
{
    ...
    private readonly IAttachmentService<TAttachmentKey> _attachmentService;
    private readonly IIdentifierGenerator<TEntityAttachmentKey, TObjectKey, TAttachmentKey> _identifierGenerator;
    public EntityAttachmentRepository(TContext dbContext, IAttachmentService<TAttachmentKey> attachmentService, IIdentifierGenerator<TEntityAttachmentKey, TObjectKey, TAttachmentKey>? identifierGenerator = null)
        : base(dbContext)
    {
        _attachmentService = attachmentService;
        _identifierGenerator = identifierGenerator ?? new DefaultIdentifierGenerator<TEntityAttachmentKey, TObjectKey, TAttachmentKey>();
    }
    ...
    public override async Task<TEntityAttachment?> Details(TEntityAttachmentKey id)
    {
        ...
        if (item != null)
        {
            item.Attachment!.Bytes = await _attachmentService.GetBytes(item.Attachment);
        }
        return item;
    }
    public override async Task<IList<TEntityAttachment>> List(TSearchObject? so = null, PagingInfo? pagingInfo = null)
    {
        ...
        {
            _attachmentService.ProcessItem(item.Attachment!);
        }
        return items;
    }
    ...
    public override async Task Add(TEntityAttachment item)
    {
        ...
        await _attachmentService.SaveFile(item.Attachment);
        await base.Add(item);
    }
    ...
    {
        await _attachmentService.RemoveFile(item.Attachment ?? new Attachment<TAttachmentKey> { Id = item.AttachmentId });
