...
using Regira.Video.FFMpeg;

var manager = new VideoManager(new ImageService());
...
foreach (var file in files)
{
    ...
    try
    {
        using var img = await manager.Snapshot(mp4File, new Regira.Dimensions.Size2D(640, 480), TimeSpan.FromSeconds(.5));
    ...
    try
    {
        using var compressedFile = await manager.Compress(mp4File);
